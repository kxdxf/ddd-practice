...
RSpec.describe UserService, "#exists?" do
  ...
  context 'userテーブルにデータが存在する' do
    before do
      ...
      (1..3).each do |num|
        database.execute(sql, "user_#{num}")
      end
    end
    ...
    it '重複するユーザー名がDBに存在する場合、trueが返却される' do
      user_1 = User.new(nil, 'user_1')
      expect(described_class.new.exists?(user_1)).to be(true)
    end

    it '重複するユーザー名がDBに存在しない場合、falseが返却される' do
      user_10 = User.new(nil, 'user_10')
      expect(described_class.new.exists?(user_10)).to be(false)
    end
  end
end